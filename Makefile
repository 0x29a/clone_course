...
isort_check: ## check that isort has been run
	isort --check-only clone_course/

isort: ## run isort to sort imports in all Python files
	isort --recursive --atomic clone_course/

style: ## run Python style checker
	pylint --rcfile=pylintrc clone_course *.py
...
pii_check: ## check for PII annotations on all Django models
	DJANGO_SETTINGS_MODULE=clone_course.settings.test \
...
dummy_translations: ## generate dummy translation (.po) files
	cd clone_course && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it clone_course /edx/app/clone_course/devstack.sh open

pkg-devstack: ## build the clone_course image from the latest configuration and code
	docker build -t clone_course:latest -f docker/build/clone_course/Dockerfile git://github.com/openedx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd clone_course && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/clone_course
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it clone_course.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it clone_course.db mysql -u root clone_course
...
%-attach:
	docker attach clone_course.$*

github_docker_build:
	docker build . -f Dockerfile --target app -t openedx/clone_course

github_docker_tag: github_docker_build
	docker tag openedx/clone_course openedx/clone_course:${GITHUB_SHA}

github_docker_auth:
	echo "$$DOCKERHUB_PASSWORD" | docker login -u "$$DOCKERHUB_USERNAME" --password-stdin

github_docker_push: github_docker_tag github_docker_auth ## push to docker hub
	docker push 'openedx/clone_course:latest'
	docker push "openedx/clone_course:${GITHUB_SHA}"
